...
NoiseProc::NoiseProc() : Processor("noise") {
  ...
  // Separate rates for 17inch and 20inch pmts
  fNoise17 = lnoise->GetD("noise_rate17");
  fNoise20 = lnoise->GetD("noise_rate20");
...
Processor::Result NoiseProc::DSEvent(DS::Root* ds) {
  ...
  double dark17Count = darkCount - darkCount + fNoise17 * noiseWindowWidth * 1325 * 1e-9;
  double dark20Count = fNoise20 * noiseWindowWidth * 554 * 1e-9;

//  int noiseHits = static_cast<int>(floor(CLHEP::RandPoisson::shoot(darkCount)));
  int noise17 = static_cast<int>(floor(CLHEP::RandPoisson::shoot(dark17Count)));
  int noise20 = static_cast<int>(floor(CLHEP::RandPoisson::shoot(dark20Count)));
  // Let's have two loops one for 17inch and another for 20inch
...
//  }
  ...
  for(int ihit=0; ihit < noise17; ihit++){ // 17inch pmt noise loop
    int pmtid = static_cast<int>(G4UniformRand() * 1325);
    if( !mcpmtObjects.count(pmtid) ) // If the PMT is not found, should never happen
	{
      DS::MCPMT* mcpmt = mc->AddNewMCPMT();
      mcpmtObjects[pmtid] = mc->GetMCPMTCount() - 1;
      mcpmt->SetID(pmtid);
  ...
  for(int ihit=0; ihit < noise20; ihit++){ // 20inch pmt noise loop
    int pmtid = static_cast<int>(G4UniformRand() * 554) + 1325;
    if( !mcpmtObjects.count(pmtid) ) // If the PMT is not found, should never happen
    {
      DS::MCPMT* mcpmt = mc->AddNewMCPMT();
      mcpmtObjects[pmtid] = mc->GetMCPMTCount() - 1;
...
void NoiseProc::SetD(std::string param, double value)
{
  ...
  else if (param == "rate17")
    if(value > 0)
      fNoise17 = value;
    else
      throw ParamInvalid(param, "Noise rate must be positive");
  else if (param == "rate20")
    if(value > 0)
      fNoise20 = value;
    else
      throw ParamInvalid(param, "Noise rate must be positive");
